...
function refreshData(){
    ...
    EGGS_TO_HATCH_1SQUIRREL(function(eggs){
        eggstohatch1=eggs
    });
    getMyEggs(function(eggs){
        ...
        secondsuntilfull=eggstohatch1-eggs/lastNumSquirrel
        console.log('secondsuntilfull ',secondsuntilfull,eggstohatch1,eggs,lastNumSquirrel)
        lastSecondsUntilFull=secondsuntilfull
        timeuntilfulldoc.textContent=secondsToString(secondsuntilfull)
        if(lastNumSquirrel==0){
            timeuntilfulldoc.textContent='?'
        }
    });
    getMySquirrel(function(squirrel){
        lastNumSquirrel=squirrel
        var gfsdoc=document.getElementById('getfreesquirrel')
        if(squirrel>0){
        ...
        var allnumsquirrel=document.getElementsByClassName('numsquirrel')
        for(var i=0;i<allnumsquirrel.length;i++){
            if(allnumsquirrel[i]){
                allnumsquirrel[i].textContent=translateQuantity(squirrel,0)
            }
        }
        var productiondoc=document.getElementById('production')
        productiondoc.textContent=formatEggs(lastNumSquirrel*60*60)
    });
    updateBuyPrice()
    updateSellPrice()
	updateSquirrelmasterPrice()
	updateCurrentSquirrelmaster()
...
function updateEggNumber(eggs){
    var hatchsquirrelquantitydoc=document.getElementById('hatchsquirrelquantity')
    hatchsquirrelquantitydoc.textContent=translateQuantity(eggs,0)
...
function liveUpdateEggs(){
    if(lastSecondsUntilFull>1 && lastNumEggs>=0 && lastNumSquirrel>0 && eggstohatch1>0){
        currentTime=new Date().getTime()
        if(currentTime/1000-lastHatchTime>eggstohatch1){
            return;
        }
        difference=(currentTime-lastUpdate)/1000
        additionalEggs=Math.floor(difference*lastNumSquirrel)
...
function updateSquirrelmasterPrice(){
    var squirrelmasterpricedoc=document.getElementById('squirrelmasterprice')
	getSquirrelmasterReq(function(req) {
		squirrelmasterpricedoc.textContent = translateQuantity(req, 0);
	});
}


function updateCurrentSquirrelmaster(){
    var currentsquirrelmasterdoc=document.getElementById('currentsquirrelmaster')
    //eggstobuydoc.textContent='?'
	currentsquirrelmaster.textContent=ceoAddress()
}

function getFreeSquirrel2(){
    var ethtospenddoc=0.001//document.getElementById('freesquirrelspend')
    weitospend=web3.toWei(ethtospenddoc,'ether')
    getFreeSquirrel(weitospend,function(){
...
function disableButtons(){
    var allnumsquirrel=document.getElementsByClassName('btn-lg')
    for(var i=0;i<allnumsquirrel.length;i++){
        if(allnumsquirrel[i]){
            allnumsquirrel[i].style.display="none"
        }
    }
}
function enableButtons(){
    ...
    for(var i=0;i<allnumsquirrel.length;i++){
        if(allnumsquirrel[i]){
            allnumsquirrel[i].style.display="inline-block"
        }
    }
}
